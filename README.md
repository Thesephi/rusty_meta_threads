...
```rs
use rusty_meta_threads as rmt;

//------------------------------
// client-side
let login_url = rmt::get_threads_login_url();
...
let code = rmt::get_code_from_redirect_uri("<REDIRECT_URL>");

// `code` can now be exchanged for a short-lived bearer token
let short_lived_token = rmt::get_short_lived_bearer_token(&code)
    .await
    .unwrap()
    .access_token;
...
let long_lived_token = rmt::get_long_lived_bearer_token(&short_lived_token)
    .await
    .unwrap()
    .access_token;
...
let profile_info = rmt::get_profile_info(&long_lived_token)
    .await
    .unwrap();

// long-lived token can be refreshed
// as long as it doesn't expire yet
let refreshed_token = rmt::refresh_long_lived_bearer_token(&long_lived_token)
    .await
    .unwrap()
    .access_token;
```
