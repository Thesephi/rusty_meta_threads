...
let code =
    rusty_meta_threads::get_code_from_redirect_uri("<REDIRECT_URL>");
...
let long_lived_token =
    rusty_meta_threads::get_long_lived_bearer_token(&short_lived_token)
        .await
        .unwrap()
        .access_token;
...
let profile_info =
    rusty_meta_threads::get_profile_info(&long_lived_token)
        .await
        .unwrap();
...
let refreshed_token =
    rusty_meta_threads::refresh_long_lived_bearer_token(&long_lived_token)
        .await
        .unwrap()
        .access_token;
```
